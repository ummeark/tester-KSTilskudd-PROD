...
// Cookies som tilhører PROD-portalen (tilskudd.fiks.ks.no / fiks.ks.no)
const PROD_DOMENER = ['tilskudd.fiks.ks.no', 'fiks.ks.no'];
const erProdCookie = (c) => PROD_DOMENER.some(d => (c.domain || '').includes(d));
...
<div class="hoveddel">
  ...
  <details style="margin-top:2rem;border:1px solid #e5e3de;border-radius:.5rem;padding:1rem 1.2rem;background:#fafaf9">
    ...
    <div style="margin-top:1rem;display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,1fr));gap:1.2rem;font-size:.82rem">
      <div>
        <div style="font-weight:600;color:#0a1355;margin-bottom:.4rem">📋 HTTP-sikkerhetshoder</div>
        <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:.2rem">
          ${['HSTS (Strict-Transport-Security)','Content-Security-Policy','X-Frame-Options','X-Content-Type-Options','Referrer-Policy','Permissions-Policy','COOP (Cross-Origin-Opener-Policy)','CORP (Cross-Origin-Resource-Policy)'].map(n => `<li style="color:#374151">· ${n}</li>`).join('')}
        </ul>
      </div>
      <div>
        <div style="font-weight:600;color:#0a1355;margin-bottom:.4rem">🔒 HTTPS &amp; stier</div>
        <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:.2rem">
          ${['HTTP → HTTPS-omdirigering','Sensitive stier: /api, /admin, /env, /.git','Sensitive stier: /robots.txt, /backup, /config','Mixed content (HTTP-ressurser på HTTPS)'].map(n => `<li style="color:#374151">· ${n}</li>`).join('')}
        </ul>
      </div>
      <div>
        <div style="font-weight:600;color:#0a1355;margin-bottom:.4rem">🍪 Cookies &amp; klient</div>
        <ul style="list-style:none;padding:0;margin:0;display:flex;flex-direction:column;gap:.2rem">
          ${['Cookie: SameSite-attributt','Cookie: Secure-flagg','Cookie: HttpOnly-flagg','Input-refleksjon / XSS i søkefelt','CORS: Access-Control-Allow-Origin'].map(n => `<li style="color:#374151">· ${n}</li>`).join('')}
        </ul>
      </div>
    </div>
  </details>
